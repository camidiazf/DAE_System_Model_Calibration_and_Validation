...
from Aux_Functions import define_cost_function, format_number, append_csv_row, init_csv_with_header, validation_analysis, parameter_analysis
from Plotting_functions import figures_dir_from_csv_path, plot_residuals_summary, plot_sensitivity_summary, plot_corr_summary
...
def RUN_PARAMETERS_ITERATIONS(iterations: int, 
    ...
    Parameters:
        ...
        - new_og: Optional dictionary of new original parameters to use instead of the default ones.
    """
    ...
    # Update original parameters if new_og is provided
    parameters_og_new = {}
    ...
    print(pd.DataFrame(parameters_og_new, index=[0]).T.rename(columns={0: 'Original Parameters'}))
        
    # Prepare CSV path and figure output directory
    csv_path = os.path.join(os.getcwd(), folder, file_name)
    fig_outdir = figures_dir_from_csv_path(csv_path) 
    os.makedirs(folder, exist_ok=True)
    init_csv_with_header(csv_path, column_names)

    # Run initial analysis with original parameters
    initial_results = RUN_INITIAL(fig_outdir=fig_outdir, new_og=new_og) 
    row_data = ['Original', '', '', '']
...
def RUN_INITIAL(fig_outdir: str, new_og: Optional[Dict[str, float]] = None) -> List[Any]:
    """
    ...
    # Update original parameters to given new_og if provided
    parameters_og_new = {}
...
def RUN_PSO_CALIBRATION(iteration: int, 
    ...
    # Get original parameters and update if new_og is provided
    parameters_og = system_data['parameters']
    parameters_og_new = {}
    if new_og is not None:
        for key, value in parameters_og.items():
...
def RUN_ANALYSIS(iteration: int, parameters: dict, params_list: list, fig_outdir: str, new_og=None) -> Dict[str, Any]:
    """
    ...
    if param_analysis is None:
        ...
        t_values = [None] * len(parameters_og_list)
        corr_matrix = None
        sensitivity_df = None
    else:
