...
# Analysis functions
def validation_analysis(system_data: Dict[str, any],
    ...
    plot_residuals_analysis(fig_outdir, iteration, residuals)

    # Plotting comparison of original and new parameters with validation data
    plotting_comparison(system_data=system_data,
...
def parameter_analysis(system_data: Dict[str, any],
    ...
    sensitivity = compute_sensitivity(system_data=system_data,
                                        simulate_model=simulate_model,
                                        fig_outdir = fig_outdir,
                                        iteration = iteration,
                                        x0 = x0_sim,
    ...
    FIM = compute_FIM(system_data=system_data,
                        simulate_model=simulate_model,
                        fig_outdir=fig_outdir,
                        iteration=iteration,
                        x0= x0_exp, 
...
def compute_FIM(system_data: Dict[str, any],
                simulate_model:callable,
                fig_outdir: str,
                iteration: int, 
                x0: np.ndarray, 
    ...
    for i, key in enumerate(parameters_og_list):
        result = sim_plus_minus(system_data=system_data, 
                                simulate_model=simulate_model,
                                key=key, x0=x0, parameters=parameters, time_stamps=time_stamps)
        if result is None:
    ...
    # Correlation matrix
    corr_matrix = compute_correlation_matrix(system_data=system_data, fig_outdir=fig_outdir, iteration=iteration, FIM=FIM)

    # Compute t-values
    t_values_complete = compute_t_values(system_data=system_data, iteration=iteration, parameters=parameters, FIM=FIM)
...
def compute_correlation_matrix(system_data: Dict[str, any],
    ...
    # Plot heatmap
    plot_corr_matrix(fig_outdir, iteration, corr, parameters_og_list)
...
def compute_sensitivity(system_data: Dict[str, any],
    ...
    plot_sensitivity_analysis(fig_outdir, iteration,sensitivity_df, top5_plot_df, var_names)
